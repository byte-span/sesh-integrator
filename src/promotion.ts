...
export async function promoteValidatedCommit(
  ...
  if (current === validatedCommit) {
    ...
    if (holders.length === 0) {
      if (requireCheckedOutTarget) {
        throw new PromotionBlockedError(
          `Target branch ${branch} must be checked out in one clean worktree to run post-integration commands. Check it out, then run seshx resume from the source worktree.`,
        );
      }
      return undefined;
    }
  ...
  if (
    ...
    throw new PromotionBlockedError(
      `Target worktree ${holder.path} is dirty or has an unfinished merge. Clean it without discarding user changes, then run seshx resume from the source worktree. Validated commit: ${validatedCommit}.`,
    );
  }
